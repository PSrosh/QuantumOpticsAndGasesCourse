...
# Auto save and launch P452 Project 2 Streamlit app

import os
import sys
import subprocess

# Full P452 Project 2 Streamlit Simulation (Heisenberg & Bose-Fermi)
# Notice the 'r' before the triple quotes below to make it a raw string!
...
if page == "Phase 1: Heisenberg Model":
    ...
    * **Analysis:** At $H=0$, the ground state is in the $\pm 1/2$ sectors with $E = -3J/4$. It is 4-fold degenerate. At high $H \gg J$, the field dominates, and the fully polarized state $|\downarrow\downarrow\downarrow\rangle$ becomes the unique ground state. The 4-fold degeneracy at $H=0$ physically represents **frustration**: on a triangle, you cannot anti-align all three spins simultaneously without paying an energy penalty.
    ...
    st.markdown("---")
    st.subheader("Interactive Numerical Simulation: Energy Spectra & Magnetization")
    
    col1, col2 = st.columns(2)
    with col1:
        N_selected = st.selectbox("Number of Spins (N)", [4, 16], index=1)
    ...
    with col2:
        ...
        num_points = st.slider("Resolution (Points)", min_value=10, max_value=200, value=100)

    # Core Logic
    @st.cache_data
    def get_basis_sz(N, k):
        """Ultra-fast basis generation using itertools"""
        basis = []
        for bits in itertools.combinations(range(N), k):
            state = sum(1 << b for b in bits)
            basis.append(state)
        return basis

    @st.cache_data
    def build_hamiltonian_H0(N, J, Sz_target):
        """Builds Hamiltonian strictly at H=0 to utilize linear scaling."""
        L = int(np.sqrt(N))
        bonds = []
        for i in range(L):
            for j in range(L):
    ...
    if st.button("Run ED Simulation"):
        with st.spinner(f"Calculating Exact Diagonalization for N={N_selected}..."):
            ...
            energies_per_sector = {}
            # Diagonalize once per sector at H=0
            for Sz in range(-N_selected//2, N_selected//2 + 1):
                H_mat = build_hamiltonian_H0(N_selected, J_val, Sz_target=Sz)
                ...
                # Minimum energy at H=0 for this sector
                E0_H0 = np.min(evals)
                # Linearly project across all H values
                energies_per_sector[Sz] = E0_H0 + H_vals * Sz
            
            # Find the global ground state and magnetization for each H
            global_gs_energy = []
            magnetizations = []
            ...
            for i in range(len(H_vals)):
                min_e = float('inf')
                best_Sz = 0
                for Sz, energies in energies_per_sector.items():
                    if energies[i] < min_e:
                        min_e = energies[i]
                        best_Sz = Sz
                
                global_gs_energy.append(min_e)
                magnetizations.append(abs(best_Sz) / (N_selected/2))
            ...
            for Sz, energies in energies_per_sector.items():
                ax1.plot(H_vals / J_val, energies, alpha=0.5, label=f'Sz={Sz}')
            ax1.plot(H_vals / J_val, global_gs_energy, 'k--', linewidth=2, label='Global GS')
            ax1.set_xlabel('H / J')
            ax1.set_ylabel('Energy E')
            ax1.set_title(f'Energy Spectrum (N={N_selected})')
            # ax1.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
            ax1.grid(True)
            
            # Plot 2: Magnetization
            ax2.plot(H_vals / J_val, magnetizations, marker='.', linestyle='-', color='b')
            ax2.set_xlabel('H / J')
            ax2.set_ylabel(r'Normalized Magnetization $\langle M_z \rangle$')
            ax2.set_title(f'Magnetization Plateaus (N={N_selected})')
            ax2.grid(True)
    ...
    * **Repulsive ($g_{BF} > 0$):** The species repel. If $g_{BF}$ is large enough (violating stability), they undergo **phase separation**. The denser gas (usually the BEC) sits at the center, acting as a "plunger" pushing the fermions into a shell at the edges of the trap.
    ...
    def solve_densities(g_BF_val, max_iter=500, mix=0.1):
        ...
        for _ in range(max_iter):
            ...
            # Mixing for numerical stability
...
folder = "/Users/pouyasadeghpour/Desktop/Desktop/PhD/UChicago/Quantum_Optics_Gases/Project2"  
filename = "streamlit_app_p452_proj2.py"
filepath = os.path.join(folder, filename)

# Create the directory if it doesn't exist to prevent file write errors
os.makedirs(folder, exist_ok=True)

with open(filepath, "w") as f:
    f.write(streamlit_code)

print(f"Streamlit app saved to: {filepath}")

print("To run your app manually, open terminal and execute:")
print(f"  streamlit run {filepath}\n")

# Auto-launch the app
try:
    subprocess.Popen([sys.executable, "-m", "streamlit", "run", filepath])
    print("Streamlit app launching...")
